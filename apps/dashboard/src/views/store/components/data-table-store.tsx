...
import { useDeleteStore } from '../api/store.mutation';
import { useGetAllStore } from '../api/store.query';
...
export function DataTableStore() {
  ...
  const storeResponse = useGetAllStore({
    limit: pagination.pageSize,
    page: pagination.pageIndex + 1,
  });
  const { paginationRange, hasPrevious, hasNext } = usePagination({
    currentPage: storeResponse.data?.meta?.page || 1,
    totalPages: storeResponse.data?.meta?.totalPages || 1,
  });

  const table = useReactTable({
    data: storeResponse.data?.data || [],
    columns,
    rowCount: storeResponse.data?.meta?.totalItems,
                ...
                {table.getHeaderGroups().map((headerGroup) => (
                  <TableRow key={headerGroup.id}>
                    {headerGroup.headers.map((header) => {
                      return (
                        <TableHead key={header.id} style={{ width: header.getSize(), maxWidth: header.getSize() }}>
      ...
      <AlertDialog open={showDialogDelete} onOpenChange={setShowDialogDelete}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>Are you sure want to delete this store?</AlertDialogDescription>
          </AlertDialogHeader>
